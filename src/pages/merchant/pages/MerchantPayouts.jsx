...
const MerchantPayouts = () => (
  <div className="animate-[fadeUp_0.4s_ease_both]">
    <MerchantPageHeader title="Payout" highlight="History" subtitle="Track all your settlement payouts" />
    <div className="mb-5 grid grid-cols-1 gap-4 min-[640px]:grid-cols-2">
      <div className="bg-card rounded-md border border-white/[0.07] p-5">
        <h3 className="mb-3 font-['Syne'] text-[1rem] font-bold text-white">Bank Account</h3>
        <div className="text-gray mb-1 text-[0.875rem]">Connected Account</div>
        <div className="mt-2 flex items-center gap-3">
          <Landmark size={28} className="text-teal" />
          <div>
            <div className="text-[1rem] font-medium text-white">Chase Bank ···· 4291</div>
            <div className="text-gray text-[0.72rem]">Routing: ···· 8742</div>
          </div>
        </div>
        <button className="text-gray2 hover:border-teal hover:text-teal mt-4 rounded border border-white/[0.07] px-3 py-1.5 text-[0.875rem]">
          Change Account
        </button>
      </div>
      <div className="bg-card rounded-md border border-white/[0.07] p-5">
        ...
        <button className="bg-teal text-navy hover:bg-teal2 mt-4 rounded px-3 py-1.5 text-[0.875rem] font-medium">
          Withdraw →
        </button>
      </div>
    </div>
          ...
          <tbody>
            {payouts.map((p) => (
              <tr key={p.ref} className="border-b border-white/[0.07] last:border-b-0">
                <td className="px-4 py-3 text-[0.875rem] text-white">{p.date}</td>
                <td className="text-gray px-4 py-3 text-[0.875rem]">{p.ref}</td>
                <td className="text-teal px-4 py-3 text-[0.875rem] font-bold">{p.amount}</td>
                <td className="text-gray px-4 py-3 text-[0.875rem]">{p.method}</td>
